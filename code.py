...
#getting api data
    ...
    for row in result:
        ...
        for row1 in api_data:
            api_id=row1["id"]
            api_name=row1["name"]
            api_data_field = row1.get("data")
            if not isinstance("api_data_field",dict):
                api_data={}
            ...
            api_price=api_data_field.get("price")
            ...
            if db_id==api_id:
                if (db_name !=  api_name or db_color !=  api_color or  db_capacity != api_capacity or  db_capacitygb !=api_capacitygb 
                    or db_price != api_price or db_generation !=api_generation or db_year != api_year or db_cpu_model != api_hard_disk_size 
                    or db_strap_color != api_strap_color or db_case_size != api_case_size or db_description != api_description):
                    ...
                    update_query="""
                        ...
                        WHERE id=?
                    """
                    # Serialize api_data to a JSON string
                    serialized_api_data = json.dumps(api_data)
                    print(serialized_api_data)
